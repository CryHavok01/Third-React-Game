...
class Room3 extends React.Component {
  render() {
    if (this.props.interactStates.garageGateOpen) {
      return(
        <div>
          You are in a parking garage.  There are a few parked cars scattered around.  Attached to the wall is an empty metal box.  The gate blocking the exit has lifted.  In the corner is a stair case leading up.
        </div>
      )
    } else if (this.props.interactStates.keyBoxOpen && this.props.inventory.includes("Car Key")) {
      return(
        <div>
          You are in a parking garage.  There are a few parked cars scattered around.  Attached to the wall is an empty metal box.  There is a metal gate blocking the exit.  Next to the gate is a card reader.  In the corner is a stair case leading up.
        </div>
      )
